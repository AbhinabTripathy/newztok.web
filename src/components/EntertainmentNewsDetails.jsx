...
const EntertainmentNewsDetails = () => {
  const { id } = useParams();
  ...
  const debug = (message, data) => {
    console.log(`[EntertainmentNewsDetails ${id}] ${message}`, data !== undefined ? data : '');
  };
  ...
  useEffect(() => {
    if (newsData && window.setDynamicMetaTags) {
      ...
      const description = newsData.content
        ...
        : 'Check out this entertainment news article from NewzTok!';
      ...
      window.setDynamicMetaTags(
        newsData.title || 'NewzTok Entertainment News Article',
        description,
        imageUrl
      );
    }
  ...
  const fetchEntertainmentNewsDetail = async () => {
    try {
      setLoading(true);
      // Try different API endpoints that might work
      const endpoints = [
        `/api/news/entertainment/${id}`,
      ...
      console.log(`Trying to fetch entertainment news article with ID: ${id}`);
      ...
      if (!response || !foundEndpoint) {
        console.log("Individual endpoints failed, trying to find article in entertainment news listing...");
        
        try {
          console.log(`Trying to find article in entertainment listing: ${baseURL}/api/news/category/entertainment`);
          const listResponse = await axios.get(`${baseURL}/api/news/category/entertainment`);
          ...
          if (listResponse.status === 200 && listResponse.data) {
            ...
            if (article) {
              console.log(`Found article in entertainment news:`, article);
              response = { 
                data: article,
                status: 200
              };
              foundEndpoint = '/api/news/category/entertainment';
            }
          }
        } catch (err) {
          console.log(`Entertainment endpoint failed: ${err.message}`);
        }
      }
      ...
      if (!response || !foundEndpoint) {
        ...
        setNewsData({
          id: id,
          title: `Entertainment News Article ${id}`,
          content: "This is sample content since the API endpoint couldn't be reached. The content would normally include details about this entertainment news article.",
          createdAt: new Date().toISOString(),
          state: "Sample State",
          district: "Sample District",
          category: "ENTERTAINMENT",
          featuredImage: "https://via.placeholder.com/800x400?text=Entertainment+News+Image"
        });
      } else {
      ...
      console.error("Error fetching entertainment news details:", err);
  ...
  const handleLikeToggle = async (event) => {
    console.log('🔥 ENTERTAINMENT LIKE BUTTON CLICKED - NO AUTH REQUIRED');
  ...
  const processContentWithAdditionalImages = (content, additionalImages) => {
    ...
    console.log("Processing entertainment content with additional images:", {
    ...
    console.log("Processed entertainment content length:", processedContent.length, "Images inserted:", imageIndex);
    return processedContent;
  };
  ...
  // Add function to check if user has already liked the article (no auth required)
    ...
    return (
      <Box sx={{ 
        ...
        <Typography variant="h5" color="error" gutterBottom>
          Entertainment article not found
        </Typography>
        <Typography variant="body1" color="textSecondary" sx={{ mb: 3, textAlign: 'center' }}>
          The entertainment article you are looking for could not be found or has been removed.
        </Typography>
        <Button 
          variant="contained" 
  ...
  const isVideoContent = mediaUrl && mediaUrl.type === 'video';
  ...
  return (
    <Box sx={{ bgcolor: 'white', minHeight: '100vh' }}>
      {/* Back Button with Entertainment Theme */}
      <Box sx={{ 
        background: `linear-gradient(135deg, ${themeColor}, #7B1FA2)`,
        color: 'white',
        py: 1.5,
        px: 2,
        position: 'relative',
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 3 }}>
          ...
          >
            {newsData.category || 'ENTERTAINMENT'}
          </Box>
            ...
            {mediaUrl.type === 'video' ? (
              ...
              <Box 
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'https://placehold.co/800x400/000000/FFFFFF/png?text=Entertainment+News+Image';
                }}
              />
            )}
          </Box>
        ...
        <Box sx={{ py: 2, display: 'flex', alignItems: 'center', gap: 4, borderBottom: '1px solid #eee', mb: 3 }}>
          <Box 
            ...
            onClick={(e) => {
              console.log('🔥 ENTERTAINMENT LIKE BOX CLICKED');
              e.preventDefault();
              e.stopPropagation();
              handleLikeToggle(e);
            }}
          >
            <IconButton 
              ...
              size="small"
              onClick={(e) => {
                console.log('🔥 ENTERTAINMENT LIKE ICON CLICKED');
                e.preventDefault();
                e.stopPropagation();
                handleLikeToggle(e);
              }}
...
export default EntertainmentNewsDetails; 