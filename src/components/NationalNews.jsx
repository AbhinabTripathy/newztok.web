...
const NationalNews = () => {
  ...
  const fetchNationalNews = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://api.newztok.in/api/news/category/national');
  ...
  const NewsCard = ({ item }) => {
    // Check if item has video content - same approach as in Sports.jsx
    ...
    useEffect(() => {
      ...
      if (hasVideo) {
        console.log(`Video detected for "${item.title}": ${videoUrl}`);
      }
    }, [mediaUrl, item.title, hasVideo, videoUrl]);
          ...
          >
            ...
            <Box
              sx={{
                ...
                left: 16,
                zIndex: 2,
                backgroundColor: hasVideo ? '#E53E3E' : '#FF6F00',
                color: 'white',
                fontWeight: 'bold',
                fontSize: '0.75rem',
                padding: '6px 16px',
