...
const NationalNewsDetails = () => {
  ...
  const fetchNationalNewsDetail = async () => {
    try {
      ...
      console.log(`Trying to fetch national news article with ID: ${id}`);
      ...
      if (!response || !foundEndpoint) {
        console.log("Individual endpoints failed, trying to find article in national news listing...");
        
        try {
          console.log(`Trying to find article in national listing: ${baseURL}/api/news/category/national`);
          const listResponse = await axios.get(`${baseURL}/api/news/category/national`);
          ...
          if (listResponse.status === 200 && listResponse.data) {
            ...
            if (article) {
              console.log(`Found article in national news:`, article);
              response = { 
                data: article,
                status: 200
              };
              foundEndpoint = '/api/news/category/national';
            }
          }
        } catch (err) {
          console.log(`National endpoint failed: ${err.message}`);
        }
      }
      ...
      if (!response || !foundEndpoint) {
        ...
        setNewsData({
          id: id,
          title: `National News Article ${id}`,
          content: "This is sample content since the API endpoint couldn't be reached. The content would normally include details about this national news article.",
          createdAt: new Date().toISOString(),
          state: "Sample State",
          district: "Sample District",
          category: "NATIONAL",
          featuredImage: "https://via.placeholder.com/800x400?text=National+News+Image"
        });
      } else {
        ...
        if (response.data) {
          ...
          console.log("Setting national news data:", articleData);
          
          // Log additional images specifically
          if (articleData.additionalImage && Array.isArray(articleData.additionalImage)) {
            console.log("Additional images found in national news:", articleData.additionalImage.length, articleData.additionalImage);
          } else {
            console.log("No additional images found in national news or not an array:", articleData.additionalImage);
          }
      ...
      console.error("Error fetching national news details:", err);
  ...
  const handleLikeToggle = async (event) => {
    console.log('🔥 NATIONAL LIKE BUTTON CLICKED - NO AUTH REQUIRED');
  ...
  const getMediaUrl = (item) => {
    ...
    return {
      type: 'image',
      url: 'https://placehold.co/800x400/000000/FFFFFF/png?text=National+News+Image'
    };
  };
    ...
    return (
      <Box sx={{ 
        ...
        <Typography variant="h5" color="error" gutterBottom>
          National article not found
        </Typography>
        <Typography variant="body1" color="textSecondary" sx={{ mb: 3, textAlign: 'center' }}>
          The national article you are looking for could not be found or has been removed.
        </Typography>
        <Button 
          variant="contained" 
  ...
  return (
    <Box sx={{ bgcolor: 'white', minHeight: '100vh' }}>
      {/* Back Button with National Theme */}
      <Box sx={{ 
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            {mediaUrl.type === 'video' ? (
              ...
              <Box 
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'https://placehold.co/800x400/000000/FFFFFF/png?text=National+News+Image';
                }}
              />
            )}
          </Box>
        ...
        <Box sx={{ py: 2, display: 'flex', alignItems: 'center', gap: 4, borderBottom: '1px solid #eee', mb: 3 }}>
          <Box 
            ...
            onClick={(e) => {
              console.log('🔥 NATIONAL LIKE BOX CLICKED');
              e.preventDefault();
              e.stopPropagation();
              handleLikeToggle(e);
            }}
          >
            <IconButton 
              ...
              size="small"
              onClick={(e) => {
                console.log('🔥 NATIONAL LIKE ICON CLICKED');
                e.preventDefault();
                e.stopPropagation();
                handleLikeToggle(e);
              }}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer', color: '#25D366' }} onClick={handleShareClick}>
            <IconButton color="default" size="small" sx={{ color: '#25D366', '&:hover': { transform: 'scale(1.1)' } }}>
              <WhatsAppIcon />
            </IconButton>
            <Typography variant="body2" sx={{ ml: 0.5 }}>
              Share on WhatsApp
        ...
        <Box 
          ...
          dangerouslySetInnerHTML={{ 
            __html: processContentWithAdditionalImages(
              newsData.content || "No content available for this national article.",
