...
const SportsNewsDetails = () => {
  ...
  const debug = (message, data) => {
    console.log(`[SportsNewsDetails ${id}] ${message}`, data !== undefined ? data : '');
  };
  ...
  useEffect(() => {
    if (newsData && window.setDynamicMetaTags) {
      ...
      const description = newsData.content
        ...
        : 'Check out this sports news article from NewzTok!';
      ...
      window.setDynamicMetaTags(
        newsData.title || 'NewzTok Sports News Article',
        description,
        imageUrl
      );
    }
  ...
  const handleLikeToggle = async (event) => {
    console.log('🔥 SPORTS LIKE BUTTON CLICKED - NO AUTH REQUIRED');
  ...
  const handleShareClick = () => {
    try {
      // Get the title
      const title = newsData?.title || 'NewzTok Sports Article';
      
      // Create the proper news URL with the ID
      const newsUrl = `https://newztok.in/sports/${id}`;
  ...
  const processContentWithAdditionalImages = (content, additionalImages) => {
    ...
    console.log("Processing sports content with additional images:", {
    ...
    console.log("Processed sports content length:", processedContent.length, "Images inserted:", imageIndex);
    return processedContent;
  };
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Box sx={{ py: 2, display: 'flex', alignItems: 'center', gap: 4, borderBottom: '1px solid #eee', mb: 3 }}>
          <Box 
            ...
            onClick={(e) => {
              console.log('🔥 SPORTS LIKE BOX CLICKED');
              e.preventDefault();
              e.stopPropagation();
              handleLikeToggle(e);
            }}
          >
            <IconButton 
              ...
              size="small"
              onClick={(e) => {
                console.log('🔥 SPORTS LIKE ICON CLICKED');
                e.preventDefault();
                e.stopPropagation();
                handleLikeToggle(e);
              }}
